...
fn fft_inverse(data: &[Complex32]) -> Vec<f32> {
    let mut input = data.to_vec();
    let planner = realfft::RealFftPlanner::<f32>::new()
        .plan_fft_inverse(input.len() * 2 - 1);
    let mut output = planner.make_output_vec();
    planner.process(&mut input, &mut output).unwrap();
    output
}
...
pub fn calculate_step_response(
    ...
    let frequency_response: Vec<_> = input_spectrum.iter()
    ...
    let step_response: Vec<_> = impulse_response.iter()
        .scan(0.0, |cum_sum, x| {
            *cum_sum += *x;
            Some(*cum_sum)
        })
    ...
    let normalized = step_response.iter()
